...
describe('RoleResolver (e2e)', () => {
  ...
  describe('createRole', () => {
    it('should create a new role successfully', async () => {
      ...
      const savedRole = await dbService.role.findUnique({
        where: {
          id: response.body.data.createRole.id,
    ...
    it('should throw ConflictException if role name already exists', async () => {
      await dbService.role.create({
        data: { name: 'Admin', channels: { connect: { code: 'default' } } },
      })
    ...
    it('should create a role for a specific channel', async () => {
      ...
      const savedRole = await dbService.role.findUnique({
        where: {
          id: response.body.data.createRole.id,
          channels: { some: { code: 'test-channel-code' } },
        },
  ...
  describe('renameRoleById', () => {
    it('should rename a role successfully', async () => {
      const role = await dbService.role.create({
        data: { name: 'OldName', channels: { connect: { code: 'default' } } },
      })
      ...
      const renamedRole = await dbService.role.findUnique({
        where: { id: role.id, channels: { some: { code: 'default' } } },
      })
    ...
    it('should throw ConflictException if role name already exists', async () => {
      await dbService.role.create({
        data: { name: 'NewName', channels: { connect: { code: 'default' } } },
      })
      const role = await dbService.role.create({
        data: { name: 'OldName', channels: { connect: { code: 'default' } } },
      })
  ...
  describe('findRoles', () => {
    it('should retrieve all roles with pagination', async () => {
      await dbService.role.create({
        data: { name: 'Role1', channels: { connect: { code: 'default' } } },
      })
      await dbService.role.create({
        data: { name: 'Role2', channels: { connect: { code: 'default' } } },
      })
      await dbService.role.create({
        data: { name: 'Role3', channels: { connect: { code: 'default' } } },
      })
